...
def scene_detection(
    ...
    if start_frame != 0 or end_frame is not None:
        start_timecode = FrameTimecode(timecode=start_frame, fps=video_manager.get_framerate())
        end_timecode = FrameTimecode(timecode=end_frame, fps=video_manager.get_framerate())
        video_manager.set_duration(start_time=start_timecode, end_time=end_timecode)
